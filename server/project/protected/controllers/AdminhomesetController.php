...
class AdminhomesetController extends AdminSet
{
    ...
    public function actionAddManager()
    {
        ...
        $pages['numPerPage'] = Yii::app()->getRequest()->getParam("numPerPage", 50); //每页多少条数据
        $criteria = new CDbCriteria;
        $criteria->addCondition("type=8");
        $pages['countPage'] = AppJxNews::model()->count($criteria);
        $criteria->limit = $pages['numPerPage'];
        $criteria->offset = $pages['numPerPage'] * ($pages['pageNum'] - 1);
        $criteria->order = 'id DESC';
        $allList = AppJxNews::model()->findAll($criteria);
        $this->renderPartial('addmanager', array(
            'models' => $allList,
            'pages' => $pages),false,true);
    }
     ...
     * 添加新闻
     */
    public function actionAddAdd()
    {
        $this->renderPartial('addadd');
    }
    ...
    public function actionAddEdit()
    {
        ...
        if($id!="")
            $model = AppJxNews::model()->findByPk($id);
        $this->renderPartial('addedit',array("models"=>$model));
    }
    ...
    public function actionSlideUpdate()
    {
        ...
        $type = Yii::app()->getRequest()->getParam("slide_type", 1); //类型
        $title = Yii::app()->getRequest()->getParam("slide_title", ""); //标题
        $redirect = Yii::app()->getRequest()->getParam("slide_redirect", ""); //用户名
